...
{% macro module_macro(name, id, parameters, messages, modules, direction, indent) -%}{% filter indent(indent) %}new MessageType("{{ name }}", {{ id }}{% if parameters|length > 0 %}{% for item in parameters|direction(direction) %}, new ParameterType("{{ item.parameter.name }}", {{ item.parameter.type|datatype }}){% endfor %}{% endif %}
																{% if messages|length > 0 %}{% for item in messages|direction(direction) %}, new MessageType("{{ item.name }}", {{ item.id }}{% if item.parameters|length > 0 %}{% for param in item.parameters %}, new ParameterType("{{ param.name }}", {{ param.type|datatype }}){% endfor %}{% endif %}){% endfor %}{% endif %}{% for module in modules %}, {{ module_macro(module.name, module.id, module.parameters, module.messages, module.modules, direction, indent+4 ) }}{% endfor %})
{% endfilter %}{%- endmacro %}
